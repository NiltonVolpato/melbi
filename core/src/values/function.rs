...
pub trait AnnotatedFunction<'types>: Function<'types, 'types> {
    ...
    fn register(
        self,
        arena: &'types Bump,
        builder: super::dynamic::RecordBuilder<'types, 'types>,
    ) -> Result<super::dynamic::RecordBuilder<'types, 'types>, super::from_raw::TypeError>
    where
        Self: Sized + 'types,
    {
        let name = self.name().to_string();
        let value = Value::function(arena, self)?;
        Ok(builder.field(name.as_str(), value))
    }
}
